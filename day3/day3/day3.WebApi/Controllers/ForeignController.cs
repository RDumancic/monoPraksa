...
namespace day3.WebApi.Controllers
{
    public class ForeignController : ApiController
    {
        ...
        public HttpResponseMessage GetForeignData(int id)
        {
            ...
            sqlCmd.CommandText = "Select * from ForeignData where id=" + id + "";
        ...
        [HttpPost]
        public HttpResponseMessage AddForeignData([FromBody]Foreign data)
        {
            if(data != null)
            {
                if(data.Name != "")
                {
                    ...
                    SqlCommand sqlCmd = new SqlCommand();
                    sqlCmd.CommandType = CommandType.Text;
                    sqlCmd.CommandText = "INSERT INTO ForeignData (name) Values (@name)";
                    sqlCmd.Connection = myConnection;
        ...
        public HttpResponseMessage DeleteForeignData(int id)
        {
            ...
            SqlCommand sqlCmd = new SqlCommand();
            sqlCmd.CommandType = CommandType.Text;
            sqlCmd.CommandText = "delete from ForeignData where id=" + id + "";
            sqlCmd.Connection = myConnection;
            myConnection.Open();
            sqlCmd.ExecuteNonQuery();
            myConnection.Close();
        ...
        public HttpResponseMessage UpdateForeignData(int id, [FromBody] Foreign data)
        {
            if (data != null)
            {
                if (data.Name != "" || id > 1)
                {
                    ...
                    SqlCommand sqlCmd = new SqlCommand();
                    sqlCmd.CommandType = CommandType.Text;
                    sqlCmd.CommandText = "UPDATE ForeignData SET name = @name WHERE id=" + id + "";
                    sqlCmd.Connection = myConnection;
